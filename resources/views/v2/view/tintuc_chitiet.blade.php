@extends('v2/structor/main',['title'=> 'Chi tiết tin tức'])
@section('content')
<div class="container">
    <div id="top">
            ...
            <div class="posts-list">
                <ul>
                    ...
                    @if($l['article_id'] != $baiviet['article_id'])
                        <li>
                            <a href="/baiviet/{{$l->article_url}}-{{$l['article_id']}}" class="image">
                                    <img src="@if(!empty($l->image)) @if(strpos($l->image,'http')===false) /public/images/{{$l->image}} @else {{$l->image}} @endif @endif" alt="" title="">
                            </a>
                            <a href="/baiviet/{{$l->article_url}}-{{$l['article_id']}}" title="">{{$l['article_title']}}</a>
                        </li>
    ...
    <section class="supplementary" id="comment-wrapper">
        ...
        <div id="list-comment">
         ...
         @if(Session::get('user')!=null) 
            ...
            <h3><a style="font-size: 16px;" href="/home">Bạn Cần Đăng Nhập Bình Luận</a></h3>
        @endif
    </div>
                ...
                <h3>Bài đăng gần đây</h3>
                    <ul class="recent-list">       
                        @foreach($noibat as $n)
                            ...
                            <li>
                                <a class="image" href="/baiviet/{{$n->article_url}}-{{$n['article_id']}}" @if(!empty($n->image)) style="background-image: url( @if(strpos($n->image,'http')===false) /public/images/{{$n->image}} @else {{$n->image}}) @endif" @endif title="">
                                </a>
                                <div class="body">
                                    <h4>
                                        <a href="/baiviet/{{$n->article_url}}-{{$n['article_id']}}" title="">
                                           {{$n['article_title']}}
                                        </a>
                                    </h4>
